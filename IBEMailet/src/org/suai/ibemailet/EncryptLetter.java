...
public class EncryptLetter extends GenericMailet {
    ...
    public void service(Mail mail) throws MessagingException {
        ...
        if (message.isMimeType("text/plain")) {
            ...
            try {
                text = message.getInputStream();
            } catch (IOException ex) {
                Logger.getLogger(EncryptLetter.class.getName()).log(Level.SEVERE, null, ex);
            }
                System.out.println ("Encrypt mail body...");
            try {
                encrypted = client.encryptData(text, client.genPkID(recip, pkg.MPK), pkg.MPK, pkg.signKeyExtract(sender), pkg.e);
                ...
                message.setContent(ciphertext, contentType);
