...
public class Cryptocontainer {
    ...
    int writeParam(int dataSize, int firstKeySize, int secondKeySize, int encryptedDataSize, int signatureSize) {
     ...
     public Cryptocontainer GetCryptocontainerParameters(FileInputStream fin, DataInputStream ds) throws IOException {
        ...
        int check = this.writeParam(data_size, key_size1, key_size2, encrypted_data_size, sign_length);
        if (check == -1) {
            return null;
        }
        return this;
